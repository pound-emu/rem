...
void run_undefined_behavior_check_pass(ir_operation_block* ir, ir_operation_block* source) {
    ...
    for (auto i = source->operations->first->next; i != nullptr; i = i->next) {
        ...
        ir_operation_block::emit_with(ir, working_operation.instruction, working_operation.destinations.data, working_operation.destinations.count, working_operation.sources.data, working_operation.sources.count);
    }
}